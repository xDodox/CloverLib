...
function UILib.newWindow(title, size, theme, parent, showVersion, includeUITab)
    ...
    if includeUITab ~= false then
        ...
        grp:colorpicker("Accent Color", self.theme.Accent, function(c) self.theme.Accent = c; self.theme.AccentD = Color3.new(c.r*0.75, c.g*0.75, c.b*0.75) end):SetTooltip("Change accent color (requires restart)")
        grp:keybind("Toggle Key", "RightShift", function(_, name) self.toggleKey = name == "RightShift" and Enum.KeyCode.RightShift or Enum.KeyCode[name] or Enum.KeyCode.RightShift end):SetTooltip("Set key to show/hide menu")
        grp:toggle("Show Version", self.showVersion, function(v) self.showVersion = v; if v then else end end):SetTooltip("Show version pill in header")
        grp:toggle("Show Watermark", false, function(v) if v then self:addWatermark("CloverHub") else if self.watermark then self.watermark:Destroy() self.watermark = nil end end end):SetTooltip("Display FPS and ping")
        grp:button("Unload", function() self:Destroy() end):SetTooltip("Destroy the UI")
...
-- Column:addGroup (modified to use new functions and add tooltip support)
    ...
    function group:keybind(text, currentName, onChange, tooltip)
        ...
        kbtn.MouseButton1Click:Connect(function()
            ...
            con = UIS.InputBegan:Connect(function(i)
                ...
                if u == Enum.UserInputType.Keyboard then kbtn.Text = i.KeyCode.Name kbtn.TextColor3 = window.theme.Accent onChange(i.KeyCode, i.KeyCode.Name) window.configs[id].Value = i.KeyCode.Name
                elseif u == Enum.UserInputType.MouseButton2 then kbtn.Text = "RMB" kbtn.TextColor3 = window.theme.Accent onChange(Enum.UserInputType.MouseButton2, "RMB") window.configs[id].Value = "RMB"
                elseif u == Enum.UserInputType.MouseButton1 then kbtn.Text = "LMB" kbtn.TextColor3 = window.theme.Accent onChange(Enum.UserInputType.MouseButton1, "LMB") window.configs[id].Value = "LMB"
                elseif u == Enum.UserInputType.MouseButton3 then kbtn.Text = "MMB" kbtn.TextColor3 = window.theme.Accent onChange(Enum.UserInputType.MouseButton3, "MMB") window.configs[id].Value = "MMB"
...
-- SubTab:addGroup (simplified, uses same element functions as column, just different parent)
function UILib.SubTab:addGroup(title)
    ...
    -- Delegate all element methods to a temporary column-style group to avoid duplication
    local colGroup = setmetatable({}, {__index = function(_, k) return function(_, ...) return group[k] and group[k](group, ...) end end})
    for name, func in pairs({toggle=true,slider=true,dropdown=true,keybind=true,label=true,button=true,expandableToggle=true,collapsible=true,colorpicker=true,multidropdown=true,textbox=true,numberbox=true,rangeslider=true}) do
